...
import app.payload.request.SignupRequest;
import app.payload.response.JwtResponse;
import app.payload.response.MessageResponse;
import app.repository.RoleRepository;
import app.repository.UserRepository;
import app.security.jwt.JwtUtils;
import app.service.userService.UserDetailsImpl;
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "manager":
                        Role adminRole = roleRepository.findByName(RoleType.ROLE_MANAGER)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(adminRole);
                        break;
